"""
LLM-XRay: Visualizations (STEP 15 Complete)
Provides robust, interactive Plotly visualizations with comprehensive safeguards for:
- PCA dimensionality reduction with 0, 1, 2, or N tokens
- Zero-variance, singular, and identical vector handling
- Variable attention matrix shapes (1x1, MHA, GQA)
- Next-token probability bar charts and generation timeline diagrams
- Resilient error handling without chart crashes
"""
...
def plot_embeddings_2d(
    ...
    Args:
        ...
        embeddings_matrix: 2D numpy array of shape (seq_len, embedding_dim).
    ...
    # 3. Two tokens case
        ...
        try:
            ...
            # Fallback if vectors are identical or PCA fails
    ...
    # 4. Multi-token case (>= 3 tokens)
        ...
        try:
            ...
            if np.allclose(clean_mat, clean_mat[0]):
                # Add minor jitter to avoid singular matrix
            ...
            # Safe linear projection fallback
    ...
    # Scatter trace with text labels & markers
    fig.add_trace(
        go.Scatter(
            x=x_vals,
            y=y_vals,
    ...
    # Connecting path line to visualize sequence trajectory
    if n_samples > 1:
        fig.add_trace(
            go.Scatter(
                x=x_vals,
...
def plot_architecture_stack(
    ...
    Args:
        num_layers: Total number of transformer layers (e.g. 28).
        selected_layer_num: Currently active layer index (1 to num_layers).
        
    Returns:
        Plotly Figure object representing the layer pipeline.
    """
    ...
    y_positions = list(range(n_blocks))
...
def plot_attention_heatmap(
    ...
    Guaranteed safe against 1x1, 0x0, or dimensional shape variations.
    
    Args:
        tokens: List of decoded token strings.
        attention_matrix: 2D numpy array of shape (seq_len, seq_len).
        layer_num: Active transformer layer number.
        head_num: Active attention head number (None if is_average=True).
        is_average: Whether displaying mean across all heads.
    ...
    # Slice safely to square matching tokens
...
def plot_hidden_states_2d(
    ...
    Guaranteed safe against 0 tokens, 1 token, identical tokens, or singular matrices.
    
    Args:
        tokens: List of decoded token strings.
        token_ids: List of integer token IDs.
        hidden_matrix: 2D numpy array of shape (seq_len, hidden_dim).
        layer_label: Display label for the layer.
        layer_num: Layer number integer (0 for embedding, 1..28 for transformer layers).
...
def plot_next_token_probabilities(
    ...
    Args:
        top_predictions: List of dicts produced by extract_next_token_logits().
        title_suffix: Optional extra context for the title.
    ...
    fig.update_layout(
        title=dict(
            text=f"<b>Next-Token Probability Distribution (Top {len(top_predictions)} Candidates)</b><br><sup>Top Prediction: <b>'{top_1_item['token']}'</b> with <b>{top_1_item['probability_pct_str']}</b> confidence</sup>",
            x=0.02,
        ),
        xaxis=dict(
...
def plot_generation_flow(
    ...
    Args:
        prompt_tokens: List of dicts representing input prompt tokens.
        generated_tokens: List of dicts representing newly generated tokens.
    ...
    for p in prompt_tokens:
        ...
        token_ids.append(p.get("token_id", 0))
        token_types.append("Prompt Token")
        colors.append("#2c3e50")
        hover_texts.append(
    ...
    for g in generated_tokens:
        ...
        token_ids.append(g.get("token_id", 0))
        token_types.append("Generated Token")
